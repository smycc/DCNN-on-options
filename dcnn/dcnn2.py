...
#sigma_loss = [loss.detach().numpy() for loss in sigma_loss]
...
plt.xlabel('epoches')
plt.title('loss on dclstm true data')
plt.legend()
plt.show()
...
C = [loss for loss in C]
surf = ax.scatter3D(x_train[1:,0],x_train[1:,1],C[1:],color='mediumpurple') # 通过修改camp修改曲面颜色
plt.title('call_dc_lstm on true data')
plt.xlabel('tau')
plt.ylabel('M')
plt.show()
...
ax.scatter(x, y,iv_m,c = 'mediumpurple')
plt.xlabel('tau')
plt.ylabel('M')
plt.title('真实数据_隐波_dc_lstm')
plt.legend()
plt.show()
