...
class MLPLoss(nn.Module):
    ...
    def forward(self, y_test_tensor, outputs):
        return torch.mean(torch.square(outputs - y_test_tensor))
# 初始化模型，损失函数和优化器
model = MLP()
...
s = exp(-r*t)
iv_m=tensor_iv(model(X_test_tensor)[:,0], s, k, t,r)
...
iv_m=np.asarray(iv_m).reshape((-1,100))
T,M=np.meshgrid(T1,M1)
fig = plt.figure(figsize=(6, 6))
ax = fig.add_subplot(111, projection='3d')
surf = ax.plot_surface(T,M, iv_m,color='royalblue', alpha=1.0, linewidth=1.0)
plt.xlabel('tau')
plt.ylabel('M')
plt.title('生成数据_隐波')
plt.legend()
